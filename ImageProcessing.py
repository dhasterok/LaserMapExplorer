...
class ImageProcessing():
    ...
    def add_edge_detection(self):
        ...
        style = self.parent.style[self.parent.comboBoxPlotType.currentText()]
        if self.edge_img:
        ...
        if self.parent.toolButtonEdgeDetect.isChecked():
            ...
            colorlist = self.parent.get_rgb_color(self.parent.style['analyte map']['Scale']['OverlayColor'])
    ...
    def plot_gradient(self):
        ...
        cax = canvas.axes.imshow(self.grad_mag, cmap=style.get_colormap(),  aspect=aspect_ratio, interpolation='none', norm=norm)
        canvas.axes.quiver(X,Y,dx,dy, color=style.overlay_color, linewidth=0.5)
