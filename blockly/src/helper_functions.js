...
/* ---------- Category/role color + tooltip decorator ---------- */

const CAT_HUE = Object.freeze({
  GLOBAL: 360,           // Global Settings
  FILE_IO: 60,           // File I/O
  SAMPLES: 180,          // Samples & fields
  IMAGE_PROCESSING: 260, // Image Processing
  PLOT: 285,             // Plot
  MULTI: 200,            // Multidimensional
  STYLE: 300             // Style
});

// Subtle hue tweaks per "functionality role"
const ROLE_SHIFT = Object.freeze({
  ENTRY:   0,    // main/entry blocks
  OPTIONS: -10,  // option/config blocks
  STYLING: +15,  // styling chain blocks
  AUX:     +30,  // helpers
  DATA:    -20   // io/data blocks
});
// Category hues (base colors)
// Convert HSL to hex so we can feed setColour a hex for a consistent look
...
export function decorateBlock(type, { category, role = 'ENTRY', tooltip = '' } = {}) {
  ...
  def.init = function() {
    ...
    // Color
    const baseHue  = CAT_HUE[category] ?? null;
    const hueShift = ROLE_SHIFT[role] ?? 0;
    if (typeof baseHue === 'number') {
      const finalHue = (baseHue + hueShift + 360) % 360;
      const hex = hslToHex(finalHue, 62, 54); // pleasant saturation/light
      this.setColour(hex);
      this.$$category = category;
      this.$$role = role;
    }

    // Tooltip (only if empty/undefined)
    const currentTip =
      (typeof this.getTooltip === 'function' ? this.getTooltip() : null) ||
      this.tooltip_ || this.tooltip;
    if (!currentTip || String(currentTip).trim() === '') {
      const fallback = tooltip || `Block: ${type.replace(/_/g, ' ')}`;
      this.setTooltip(fallback);
    }
  };
}

/* ---------- Sample ID guard for certain blocks ---------- */

// Put this near your Blockly setup
const WHITELIST = new Set(['load_sample', 'load_directory']);
const DISABLED_REASON = 'Load sample to enable block';
export function lockdownBlocks(workspace) {
  // 1) Grey out disallowed blocks in the toolbox
  const tbx = document.getElementById('toolbox');
  if (tbx) {
    const clone = tbx.cloneNode(true);
    clone.querySelectorAll('block').forEach(node => {
      const type = node.getAttribute('type') || '';
      if (!WHITELIST.has(type)) {
        node.setAttribute('disabled', 'true');
        node.setAttribute('disabledReason', DISABLED_REASON);
      }
    });
    workspace.updateToolbox(clone);
  }

  // 2) Disable any existing blocks already on the workspace
  workspace.getAllBlocks(false).forEach(b => {
    if (!WHITELIST.has(b.type)) b.setEnabled(false);
  });

  // 3) Auto-disable any blocks created later (dragged from toolbox or via code)
  workspace.addChangeListener(function(e) {
    if (e.type !== Blockly.Events.BLOCK_CREATE) return;
    (e.ids || []).forEach(id => {
      const b = workspace.getBlockById(id);
      if (b && !WHITELIST.has(b.type)) b.setDisabledReason?.('Load sample to enable block');
    });
  });
}

function enableAllBlocks(workspace) {
  // 1) Enable blocks in the toolbox
  const tbx = document.getElementById('toolbox');
  if (tbx) {
    const clone = tbx.cloneNode(true);
    clone.querySelectorAll('block').forEach(node => {
      node.removeAttribute('disabled');
      node.removeAttribute('disabledReason');
    });
    workspace.updateToolbox(clone);
  }

  // 2) Enable blocks already in the workspace
  workspace.getAllBlocks(false).forEach(b => {
    b.setDisabledReason?.('');
  });
}
window.enableAllBlocks = enableAllBlocks;
