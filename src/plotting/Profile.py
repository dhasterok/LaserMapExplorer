...
class Profiling:
    ...
    ``src/common/PolygonMatplotlib.py``'s ``PolygonManager``.
    """
    ...
    def clear_all(self):
        """Reset to no samples/profiles and no known project directory.

        `add_samples()` only ever adds missing entries -- without this,
        closing one project and opening another in the same session would
        leak the first project's profiles into the second (called by
    ...
    def compute_profile_points(self, profile_points, radius, x, y, x_i, y_i, point_index=None):
        ...
        fields = (
            ...
            data.processed.match_attribute('data_type', 'Diffusion model')
        )
