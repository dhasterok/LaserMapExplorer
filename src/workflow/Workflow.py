...
class Workflow(CustomDockWidget):
    ...
    def __init__(self,main_window = None):
        ...
        open_icon = QIcon(":resources/icons/icon-open-file-64.svg")
        if not open_icon.isNull():
            self.open_action.setIcon(open_icon)
        else:
            self.open_action.setText("Open")
        self.open_action.setToolTip("Open workflow file")
        ...
        self.save_action.setToolTip("Save workflow to file")
        ...
        self.clear_action.setToolTip("Clear workspace")
        ...
        # Auto-load the project's active workflow file (if any) once the Blockly
        # page has actually finished loading - action capture (see
        # MainWindow._record_to_active_workflow) writes straight to that file on
        # disk whether or not this dock exists, so opening the dock should show
        # whatever's already been captured rather than starting from a blank
        # workspace.
        self.web_view.loadFinished.connect(lambda ok: self.reload_active_file() if ok else None)

    def reload_active_file(self):
        """Reload the project's active workflow file into the live Blockly workspace.

        Called whenever the active workflow file changes on disk - after
        `MainWindow._record_to_active_workflow` appends a captured action, after
        the user creates/opens a different active workflow file via
        `MainWindow.new_workflow`/`ensure_active_workflow_file`, or once when
        this dock first finishes loading - so the visible workspace never goes
        stale while the dock is open. No-op if no active file is set yet.
        """
        path = self.main_window.app_data.active_workflow_file
        if path is None or not Path(path).exists():
            return
        with open(path, 'r') as f:
            payload = json.load(f)
        blockly_state = payload.get('blockly_state', payload)
        state_str = json.dumps(blockly_state)
        self.web_view.page().runJavaScript(f"loadWorkspaceStateJSON({json.dumps(state_str)})")

    def run_workflow(self):
        """Execute the workflow, wrapped in a live `.rst` report session.

        If `main_window.report_writer` is available, a report session spans the
        run: it appends one entry per recorded action (filters, analyte-list
        changes, figures, clustering/PCA results) to the sample's Notes dock as
        the generated code executes, then compiles the notes to PDF once the
        run completes.
        """
        report_writer = getattr(self.main_window, 'report_writer', None)
        if report_writer:
            report_writer.start_session()
        try:
            self.bridge.lame_blockly.execute_code(self.output_text_edit)
        finally:
            if report_writer:
                report_writer.end_session()

    def save_workflow(self):
        """Save the current Blockly workspace to a JSON workflow file.

        Reads the serialized workspace state from the Blockly page via the
        `getWorkspaceStateJSON` JS function (added in blockly/src/app.js) and
        writes it to a user-chosen file, wrapped with a small version header
        so the format can evolve later.
        """
        path, _ = QFileDialog.getSaveFileName(self, "Save Workflow", "", "LaME Workflow (*.json)")
        if not path:
            return

        def _write(state_str):
            if not state_str:
                self.statusLabel.setText("Save failed: could not read workspace state.")
                return
            try:
                blockly_state = json.loads(state_str)
            except (TypeError, ValueError):
                self.statusLabel.setText("Save failed: invalid workspace state.")
                return
            payload = {"lame_workflow_version": 1, "blockly_state": blockly_state}
            with open(path, "w") as f:
                json.dump(payload, f, indent=2)
            self.main_window.app_data.active_workflow_file = Path(path)
            self.statusLabel.setText(f"Saved: {path}")

        self.web_view.page().runJavaScript("getWorkspaceStateJSON()", _write)

    def open_workflow(self):
        """Load a Blockly workspace from a previously saved JSON workflow file.

        Reads the file, then hands the workspace state to the `loadWorkspaceStateJSON`
        JS function (added in blockly/src/app.js), which clears the current workspace
        before loading.
        """
        ...
        try:
            ...
            self.statusLabel.setText(f"Open failed: {e}")
            return
