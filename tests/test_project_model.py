...
Runs under pytest (no QApplication needed). The ``__main__`` block below
also still runs it directly as a script -- which is why every test takes a
directory argument rather than creating its own: under pytest that argument
is filled by the ``tmp_path`` fixture, and by a tempfile when run directly.

``tmp_path`` specifically, not ``tmpdir``: these were written against
``pathlib.Path`` (``.parent``, ``/``), and pytest's legacy ``tmpdir``
fixture hands over a ``py.path.local`` instead, which has no ``.parent``.
"""
import shutil
import sys
import tempfile
...
def test_round_trip_save_load(tmp_path):
    ...
    project.workflow_refs = [tmp_path / "workflows" / "hpe_correction.json"]

    manifest = tmp_path / "projects" / "RM02study" / "RM02study.lame_project.json"
    ...
    assert reloaded.workflow_refs == [Path(tmp_path / "workflows" / "hpe_correction.json").resolve()]
    print("PASS: project round-trips through save_project/load_project")


def test_relative_paths_survive_moving_project_dir(tmp_path):
    raw = tmp_path / "data" / "RM02.lame.csv"
    raw.parent.mkdir()
    raw.write_text("Xc,Yc\n0,0\n", encoding='utf-8')
    ...
    manifest = tmp_path / "proj_original_location" / "proj.lame_project.json"
    save_project(project, manifest)

    # simulate moving the whole project directory (but not the raw data) elsewhere
    moved_dir = tmp_path / "proj_new_location"
...
def test_calibration_sidecar_round_trip(tmp_path):
    raw = tmp_path / "RM03.lame.csv"
    raw.write_text("Xc,Yc\n0,0\n", encoding='utf-8')
...
def test_calibration_schema_open_for_different_data_types(tmp_path):
