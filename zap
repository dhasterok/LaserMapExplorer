...
class ColormapPreviewWidget(FigureCanvas):
    ...
    def __init__(self, parent=None):
        ...
        # Create test data
        self.test_data = np.random.rand(50, 50)
    ...
    def update_preview(self):
        ...
        # Create two separate axes for the visualization and colorbar
        ax1 = self.figure.add_subplot(2, 1, 1)  # Top plot for the data visualization
        ax2 = self.figure.add_subplot(2, 1, 2)  # Bottom plot for colorbar
        ...
        im = ax1.imshow(self.test_data, cmap=cmap, aspect='auto')
        ax1.set_title(f'Preview ({self.preview_mode})')
        ax1.axis('off')
        
        # Create colorbar in separate axis
        ax2.axis('off')
        cbar = self.figure.colorbar(im, ax=ax2, orientation='horizontal', fraction=1.0, pad=0)
    ...
    def simulate_colorblind_view(self, cmap):
        ...
        # Simplified simulation - in practice you'd use more sophisticated algorithms
        if self.preview_mode == 'grayscale':
            # Convert to grayscale
            return plt.cm.gray
        elif self.preview_mode == 'deuteranopia':
            # Simulate deuteranopia (green-blind)
            # This is a simplified version - you'd want a proper simulation
            return cmap
        elif self.preview_mode == 'protanopia':
            # Simulate protanopia (red-blind)
            return cmap
        elif self.preview_mode == 'tritanopia':
...
class ColormapEditorDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        controls_layout.addWidget(self.discrete_checkbox)
        ...
        # Control point management buttons (for both discrete and continuous modes)
        self.control_buttons_layout = QHBoxLayout()
        
        self.add_point_button = QPushButton("Add Control Point")
        self.add_point_button.clicked.connect(self.add_control_point)
        self.control_buttons_layout.addWidget(self.add_point_button)
        
        self.delete_point_button = QPushButton("Delete Selected")
        self.delete_point_button.clicked.connect(self.delete_control_point)
        self.delete_point_button.setEnabled(False)
        self.control_buttons_layout.addWidget(self.delete_point_button)
        
        self.control_buttons_layout.addStretch()
        
        self.control_buttons_widget = QWidget()
        self.control_buttons_widget.setLayout(self.control_buttons_layout)
        display_layout.addWidget(self.control_buttons_widget)
    ...
    def on_colormap_changed(self, colormap_name: str):
        ...
        if colormap_name in self.existing_colormaps:
            ...
            colormap_data = self.existing_colormaps[colormap_name]
            # You would implement loading logic here based on your colormap format
            pass
        else:
    ...
    def on_discrete_toggled(self, is_discrete: bool):
        ...
        if is_discrete:
            ...
            self.add_point_button.setText("Add Color")
            self.delete_point_button.setText("Delete Selected Colors")
        else:
            ...
            self.add_point_button.setText("Add Control Point")
            self.delete_point_button.setText("Delete Selected Point")
    ...
    def update_button_states(self):
        ...
        if self.is_discrete:
            ...
            self.add_point_button.setEnabled(True)  # Can always add colors
            # Can delete if something is selected and won't delete all colors
            self.delete_point_button.setEnabled(selected_count > 0 and selected_count < total_colors)
        else:
            # Continuous mode
            selected_index = self.continuous_widget.get_selected_point()
            total_points = len(self.continuous_widget.get_color_points())
            
            self.add_point_button.setEnabled(total_points < 10)  # Reasonable maximum
            # Can delete if something is selected and at least 2 points will remain
            self.delete_point_button.setEnabled(selected_index >= 0 and total_points > 2)
    ...
    def update_preview(self):
        ...
        if self.preview_visible and self.current_colormap_data:
        ...
        if file_path:
            try:
                ...
                with open(file_path, 'r') as file:
                    ...
                    for row in reader:
                        if len(row) >= 3:  # Expect R, G, B (and optionally position)
                            r, g, b = float(row[0]), float(row[1]), float(row[2])
                            # Assume values are 0-1, convert to 0-255 if needed
                ...
                if colors:
                    ...
                    self.on_colormap_modified()
                    QMessageBox.information(self, "Success", f"Loaded {len(colors)} colors from CSV")
        ...
        if file_path:
            try:
                with open(file_path, 'w', newline='') as file:
                    ...
                    if self.is_discrete:
                        # Save discrete colors
                        for color in self.current_colormap_data:
                            r, g, b = color.redF(), color.greenF(), color.blueF()
                            writer.writerow([r, g, b])
                    else:
                        # Save continuous color points
                        for point in sorted(self.current_colormap_data, key=lambda x: x.position):
                            r, g, b = point.color.redF(), point.color.greenF(), point.color.blueF()
                            writer.writerow([r, g, b, point.position])
                
                QMessageBox.information(self, "Success", "Colormap saved successfully")
                self.accept()
